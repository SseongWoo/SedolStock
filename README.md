...
## 프로젝트 소개
**세돌스탁(SedolStock)**은 유튜브 채널의 활동 데이터를 기반으로 주가가 변동하는 모의 투자 시뮬레이션 게임입니다.
이 게임은 유튜브 데이터 API를 활용하여 실제 조회수, 좋아요 변화량을 바탕으로 실시간 주가 변동을 반영하며, 사용자는 이를 활용하여 투자 전략을 세울 수 있습니다.

유튜브 채널 데이터를 활용한 가상의 주식 거래

5분마다 갱신되는 실시간 데이터로 생동감 있는 투자 경험 제공

우왁굳과 이세계 아이돌 멤버 개별 채널에 대한 투자 가능

랭킹 시스템을 통해 다른 사용자와 경쟁

이 게임은 웹, Android, Windows에서 플레이할 수 있으며, Firebase 및 AWS 기반의 백엔드 인프라를 활용하여 안정적인 서비스를 제공합니다.
...
- 유튜브 API를 활용한 실시간 주가 반영
- 조회수, 좋아요 변화량을 기반으로 5분마다 가격 갱신
- 가상의 포트폴리오 관리 및 투자 전략 실행
- 실시간 랭킹 시스템을 통해 다른 사용자와 경쟁

📊 데이터 시각화

- 개별 주식의 가격 변동 차트 제공
- 보유 자산 및 투자 수익률 시각화
- 유튜브 활동 데이터와 주가 변동 간의 관계 분석 가능

🛠 플랫폼 지원

- Android, 웹, Windows 지원
- AWS 기반 서버 운영 및 백엔드 데이터 관리
- Firebase Authentication을 통한 로그인 및 보안 강화
- RESTful API를 활용한 안정적인 데이터 통신
...
- 유튜브 데이터 API를 활용한 실시간 주식 가격 변동 시스템 구현
- Firebase Firestore 및 Realtime Database를 이용한 데이터 저장 및 관리
- Flutter 기반으로 Android 및 웹에서 원활한 크로스플랫폼 운영 가능
- AWS 기반 서버 운영 및 보안 강화를 통한 안정적인 서비스 제공

## 🎓 배운 점

- Firebase 및 AWS를 활용한 백엔드 구축
- RESTful API를 통한 클라이언트-서버 통신
- 실시간 데이터 동기화 및 주식 차트 시각화
